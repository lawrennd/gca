function [A, beta, nu, sbar] = learnnoiselessica(X, latentDim);
...
for n = 1:ndata
  for j = 1:latentDim
    sigma2bar_tau(n, j) = (nu(j)*sigma2_tau(j) + WTX(n, j)*WTX(n, j))...
	/(nu(j) + 1); 
  end
end
...
for iter = 1:niters
  ...
  while maxchange > tol & counter1 < 500
    ...
    oldV = V';
    SigmaPrime = zeros(latentDim);
    for j = 1:latentDim
      for n = 1:ndata
	SigmaPrime = SigmaPrime + XULinv(n, :)'*tau(n, j)*XULinv(n, :);
      end
      [q, s] = eig(SigmaPrime);
      [sMax, index] = max(diag(s));
      V(j, :) = q(:, index)';%*sqrt(sMax);
    end
    WTX = XULinv*V;
    for n = 1:ndata
      for j = 1:latentDim
