...
nu = 1*ones(1, expectedLatentDim)*5;
...
sigma2bar_tau = rand(ndata, expectedLatentDim);
tau = 1./sigma2bar_tau;
for j = 1:expectedLatentDim
...
for iter = 1:niters
  ...
  while maxchange > tol & counter1 < 500
    ...
    oldV = V';
    sbar = updatesbar(Sigma_s, XULinv, V, tau, beta, lambda);
    V = updateV(sbar, Sigma_s, XULinv);
%    V = orthogonalise(V')';
    Sigma_s = updateSigma_s(V, tau, beta, lambda);
