...
X = foetal_ecg(:, 1:10:end);%(:, 2:9); %6500)';
%clear MEG_art;
...
for iter = 1:niters
  ...
  while maxchange > tol & counter1 < 500
    ...
    oldV = V';
    score = prod(tau, 2);
    [void, index] = sort(score);
    choose = index(1+end-numChoose:end)';

    %for i = 1:10
      sbar = updatesbar(Sigma_s, XULinv, V, tau, beta, ...
				     lambda, choose);
      V = updateV(sbar, Sigma_s, XULinv, choose);
      Sigma_s = updateSigma_s(V, tau, beta, lambda, choose);
    %end
    %[sbar, Sigma_s] = updatelatent(XULinv, V, tau, beta, lambda);
    [nubar_tau sigma2bar_tau] = stupdatetau(sbar, Sigma_s, nu, sigma2_tau);
    tau = 1./sigma2bar_tau;
  ...
  A = U*diag(lambda)*V';
  save ncompfoetalsave A beta nu
end
